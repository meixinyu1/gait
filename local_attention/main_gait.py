...
parser.add_argument("--epoch", default=40, type=int, help="epochs to run")
...
def train(model, criterion_xent, criterion_htri, criterion_bag, optimizer, trainLoader, device):
    ...
    for batch_idx, (imgs, pids, tags) in enumerate(trainLoader):
        ...
        outputs, features, local_features, n_loss = model(imgs, tags)
        ...
        if args.htri_only:
            ...
            htri_loss, features_htri_loss = criterion_htri(features, local_features, pids)
            n,p = n_loss
            # tags = tags.sum(1)
            ...
            if tags.sum() > 0:
                ...
                p = 0
            loss = xent_loss + htri_loss + features_htri_loss + p
...
def test(model, queryLoader, galleryLoader, device, ranks=[1,5,10,20]):
    with torch.no_grad():
        ...
        for batch_idx, (img, pid, bag) in enumerate(queryLoader):
            ...
            _, features, local_features = model(img)
            features = torch.cat((features, local_features), 1)
