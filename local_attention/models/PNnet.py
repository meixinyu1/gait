...
class LocalBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, downsample=True):
        super(LocalBlock, self).__init__()
        self.conv1 = conv1x1(inplanes, inplanes)
        self.bn1 = nn.BatchNorm2d(inplanes)
        self.conv2 = conv3x3(inplanes, inplanes, stride=2)
        self.bn2 = nn.BatchNorm2d(inplanes)
        self.conv3 = conv1x1(inplanes, outplanes)
        self.bn3 = nn.BatchNorm2d(outplanes)
        self.relu = nn.ReLU(inplace=True)
        self.pooling = conv3x3(inplanes, outplanes, stride=2)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        ...
        #this added
        self.local_layer1 = LocalBlock(inplanes=64, outplanes=128)
        self.local_layer2 = LocalBlock(inplanes=128, outplanes=256)
        ...
        # self.feature_softmax = nn.Softmax()
    ...
    def forward(self, x, tag=None):
        ...
        local_x = self.local_layer2(local_x)
        local_x = local_x.sum(1) / local_x.size(1)
        local_features = local_x.view(local_x.size(0), -1)
        ...
        if N:
            ...
            if tag is not None:
                ...
                # print(tag.size(), layer_bags.size())
                # tmp = layer_bags*tag
                # print(tmp.size())
        ...
        x = self.drop_out(x)
        if tag is None:
            return x, features, local_features
        if N:
            return x, features, local_features, n_loss
        else:
            return x, features, local_features
...
def resnet50(pretrained=True, num_classes=1000, user='meixinyu', **kwargs):
